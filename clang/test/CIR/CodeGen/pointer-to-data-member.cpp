...
// CIR-BEFORE: cir.func {{.*}} @_Z5test1v() -> (!cir.data_member<!s32i in !rec_Point> {cir.ast_member_pointer = {{{.*}}kind = "data"{{.*}}target_record_usr = "c:@S@Point"{{.*}}}}) attributes {{{.*}}nothrow} {
...
// CIR-AFTER: cir.func {{.*}} @_Z5test1v() -> (!s64i {cir.ast_member_pointer = {{{.*}}kind = "data"{{.*}}target_record_usr = "c:@S@Point"{{.*}}}}) attributes {{{.*}}nothrow} {
