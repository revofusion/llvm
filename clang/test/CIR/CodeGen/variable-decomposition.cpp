...
// CIR:  %[[CONST_A:.+]] = cir.const #cir.int<1> : !s32i
// CIR:  cir.store{{.*}} %[[CONST_A]], %[[MEMBER_A]]
// CIR:  %[[MEMBER_B:.+]] = cir.get_member %[[STRUCT]][1] {name = "b"} : !cir.ptr<!rec_some_struct> -> !cir.ptr<!cir.float>
// CIR:  %[[CONST_B:.+]] = cir.const #cir.fp<2.000000e+00> : !cir.float
// CIR:  cir.store{{.*}} %[[CONST_B]], %[[MEMBER_B]]
// CIR:  %[[MEMBER_A:.+]] = cir.get_member %[[STRUCT]][0] {name = "a"} : !cir.ptr<!rec_some_struct> -> !cir.ptr<!s32i>
...
// LLVM:  store float 2.000000e+00, ptr %[[GEP_B]], align 4
// LLVM:  %[[GEP_A2:.+]] = getelementptr %struct.some_struct, ptr %[[STRUCT]], i32 0, i32 0
// LLVM:  %[[LOAD_A:.+]] = load i32, ptr %[[GEP_A2]]
