...
// CHECK: %[[ITR1_LOAD:.*]] = cir.load {{.*}} : !cir.ptr<!u64i>, !u64i {{.*}}
// CHECK-NEXT: %[[INC:.*]] = cir.inc %[[ITR1_LOAD]] : !u64i
// CHECK: cir.store{{.*}} %[[INC]], %{{.*}} : !u64i, !cir.ptr<!u64i> {{.*}}
// CHECK-NEXT: cir.yield
// CHECK-NEXT: }
// CHECK-NEXT: }
// CHECK-NEXT: cir.yield
// CHECK-NEXT: } step {
// CHECK: %[[ITR2_LOAD:.*]] = cir.load {{.*}} : !cir.ptr<!u64i>, !u64i {{.*}}
...
// CHECK-NEXT: cir.yield
// CHECK-NEXT: } step {
// CHECK: %[[ITR3_LOAD:.*]] = cir.load {{.*}} : !cir.ptr<!u64i>, !u64i {{.*}}
...
// CHECK-NEXT: %[[OP:.*]] = cir.add nsw %[[LHS_LOAD]], %[[RHS_LOAD]] : !s32i
// CHECK-NEXT: cir.store{{.*}} %[[OP]], %[[LHS_GET_I]] : !s32i, !cir.ptr<!s32i>
// CHECK-NEXT: cir.yield
// CHECK-NEXT: } step {
// CHECK-NEXT: %[[ITR1_LOAD]] = cir.load %[[ITR1]] : !cir.ptr<!u64i>, !u64i
// CHECK-NEXT: %[[INC:.*]] = cir.inc %[[ITR1_LOAD]] : !u64i
...
// CHECK-NEXT: }
// CHECK-NEXT: }
// CHECK-NEXT: acc.yield %[[LHSARG]] : !cir.ptr<!cir.array<!cir.array<!cir.array<!rec_NoOps x 5> x 5> x 5>>
// CHECK-NEXT: } destroy {
