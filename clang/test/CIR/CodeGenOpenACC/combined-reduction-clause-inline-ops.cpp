...
// CHECK: cir.const {{.*}}
// CHECK: cir.store {{.*}}
// CHECK: %[[ZERO:.*]] = cir.const #false {{.*}}
// CHECK: cir.store {{.*}} %[[ZERO]], {{.*}} : !cir.bool, !cir.ptr<!cir.bool> {{.*}}
...
// CHECK-NEXT: ^bb0(%[[ARG:.*]]: !cir.ptr<!rec_HasOperatorsInline>{{.*}})
// CHECK-NEXT: %[[ALLOCA:.*]] = cir.alloca "openacc.reduction.init" {{.*}} init : !cir.ptr<!rec_HasOperatorsInline>
// CHECK: cir.get_member {{.*}}name = "i"{{.*}} : !cir.ptr<!rec_HasOperatorsInline> -> !cir.ptr<!s32i> {{.*}}
// CHECK: cir.const {{.*}}
// CHECK: cir.store {{.*}}
// CHECK: cir.get_member {{.*}}name = "u"{{.*}} : !cir.ptr<!rec_HasOperatorsInline> -> !cir.ptr<!u32i> {{.*}}
// CHECK: cir.const {{.*}}
// CHECK: cir.store {{.*}}
// CHECK: cir.get_member {{.*}}name = "f"{{.*}} : !cir.ptr<!rec_HasOperatorsInline> -> !cir.ptr<!cir.float> {{.*}}
// CHECK: cir.const {{.*}}
// CHECK: cir.store {{.*}}
// CHECK: cir.get_member {{.*}}name = "d"{{.*}} : !cir.ptr<!rec_HasOperatorsInline> -> !cir.ptr<!cir.double> {{.*}}
// CHECK: cir.const {{.*}}
// CHECK: cir.store {{.*}}
// CHECK: cir.get_member {{.*}}name = "b"{{.*}} : !cir.ptr<!rec_HasOperatorsInline> -> !cir.ptr<!cir.bool> {{.*}}
// CHECK: cir.const {{.*}}
// CHECK: cir.store {{.*}}
// CHECK-NEXT: acc.yield
//
